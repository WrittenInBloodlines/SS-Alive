...
class AliveEditorActivity : AppCompatActivity() {
    ...
    private fun buildEditor() {
        ...
        root.addView(TextView(this).apply { text = if (profile.isTemplate) "Edit Cat" else "Create Alive"; textSize = 30f })
        root.addView(TextView(this).apply { text = "Build your own screen character with PNG animation frames."; textSize = 16f; setPadding(0,12,0,20) })
        ...
        val sizeBar = SeekBar(this).apply {
            max=175; progress=profile.sizePercent.coerceIn(25,200)-25
            setOnSeekBarChangeListener(object: SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(s: SeekBar?, p: Int, fromUser: Boolean) { profile.sizePercent=p+25; updateSizeLabel() }
                override fun onStartTrackingTouch(s: SeekBar?) = Unit
                override fun onStopTrackingTouch(s: SeekBar?) = Unit
            })
        }
        root.addView(sizeBar); updateSizeLabel()
        val scroll=ScrollView(this); val states=LinearLayout(this).apply { orientation=LinearLayout.VERTICAL }; scroll.addView(states)
        AliveProfile.STATES.forEach { state ->
            ...
            stateRows[state]=row; section.addView(row); states.addView(section); refreshState(state)
        }
    ...
    private fun refreshState(state:String) {
        val row=stateRows[state] ?: return; row.removeAllViews(); val frames=profile.frameUris(state)
        row.addView(TextView(this).apply { text=if(frames.isEmpty()) "No PNG selected" else "${frames.size} frame${if(frames.size==1) "" else "s"}"; textSize=15f }, LinearLayout.LayoutParams(0,-2,1f))
        row.addView(Button(this).apply { text="+"; setOnClickListener { pendingState=state; showImageSourceDialog() } })
        if(frames.isNotEmpty()) row.addView(Button(this).apply { text="CLEAR"; setOnClickListener { profile.frames[state]?.clear(); refreshState(state) } })
    }

    private fun showImageSourceDialog() {
        AlertDialog.Builder(this).setTitle("Choose image source")
            .setItems(arrayOf("Choose from Gallery","Choose from Files")) { _,which -> if(which==0) pickImageFromGallery.launch("image/png") else pickImageFromFiles.launch(arrayOf("image/png")) }
            .setNegativeButton("Cancel",null).show()
    }

    private fun addSelectedImage(uri:Uri?) {
        if(uri==null){pendingState=null;return}; val state=pendingState ?: return
        if(uri.scheme!="content"){pendingState=null;return}
        runCatching { contentResolver.takePersistableUriPermission(uri,Intent.FLAG_GRANT_READ_URI_PERMISSION) }
        profile.frames.getOrPut(state){mutableListOf()}.add(uri.toString()); refreshState(state); pendingState=null
    }

    private fun saveProfile(useAfterSave:Boolean=false) {
        profile.name=nameInput.text.toString().trim().ifEmpty{"Alive 1"}; profile.sizePercent=profile.sizePercent.coerceIn(25,200)
        AliveRepository.save(this,profile); Toast.makeText(this,"Alive saved",Toast.LENGTH_SHORT).show()
        if(useAfterSave){ AliveRepository.setActive(this,profile); startService(Intent(this,AliveService::class.java)); finish() }
    }

    private fun confirmDelete() {
        AlertDialog.Builder(this).setTitle("Delete Alive?")
