...
class PetView(context: Context, private val profile: AliveProfile) : View(context) {
    ...
    private fun updateSpriteFrame() {
        ...
        if (stateChanged || directionChanged) {
            animationFrame = 0; lastAnimationTime = now; lastState = behavior.state; lastDirection = behavior.direction
            return
        }
        ...
        val count = PetSprite.frameCount(behavior.state).coerceAtLeast(1)
        if (now - lastAnimationTime >= frameDuration) {
            val elapsed = ((now - lastAnimationTime) / frameDuration).toInt().coerceAtMost(count)
    ...
    private fun reactToTap() {
        ...
        handler.postDelayed({
            ...
            behavior.finishReverse(); animationFrame = 0; invalidate()
        }, 220L)
    }
